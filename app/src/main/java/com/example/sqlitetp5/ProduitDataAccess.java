...
public class ProduitDataAccess {
    ...
    public ProduitDataAccess(monSqLiteOpenHelper helper) {
        this.helper = helper; db=helper.getWritableDatabase();
    }
    ...
    public ArrayList<Produit> listerProduit(){
        ...
        Cursor cursor=db.query(nom_table,new String[]{"code","description","prix"},null,null,null,null,"description desc",null);
        //rawQuery("select * from produit",null); //
        cursor.moveToFirst();
         while (!cursor.isAfterLast()){
         long code=cursor.getLong(0); String description=cursor.getString(1);
         float prix=cursor.getFloat(2);
         Produit pr=new Produit(description,prix);
         pr.setCode(code); liste.add(pr);
         cursor.moveToNext();
         }
         return liste;
    }
